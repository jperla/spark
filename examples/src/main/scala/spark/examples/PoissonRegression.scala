...
// Begin advancers
class PoissonMap (var grad: PoissonGradient, var iter:Int) extends Serializable {
    override def toString: String = {
...
object PoissonEager {
  ...
  // To the Master 
   class PoissonMapMessage (var id:Long, var message: M, @transient theType: R) extends MapMessage[M,R] {
   	 override def toString: String = { return "" } 
   }

    // From the master
    class Diff (var id:Long, @transient message: M, @transient state: R) extends EagerDiff[M,R] {
     ...
     object Reducer extends EagerReducer[M,R] {
        ...
        def mapMessage (oldVar: EagerVar[M,R], message: M) : MapMessage[M,R] = {
                val dummy:R = null
                return new PoissonMapMessage(oldVar.id, message, dummy)
            }
        ...
        def zero(initialValue: R) = initialValue.clone

    	def reduceTop (old_var: EagerVar[M,R], message: M) : EagerDiff[M,R] = {	    
            old_var.value.mutable_state.iter += 1
            var learning_rate = math.pow((old_var.value.mutable_state.iter), -1) / 1000.0
...
object AdvancersPoissonRegression {
	...
	var state = sc.eagerReducer(new PRState(examples.length, PoissonConstants.alpha, PoissonConstants.chunk_size, mutable_state), PoissonEager.Reducer)
		...
		var grad = new PoissonGradient(state.value.mutable_state.weights.num_features)
		   ...
		   state.eagerReduce(new PoissonMap(grad.clone, state.value.mutable_state.iter))
		   grad.clear()
		   var time = new Date().getTime()
