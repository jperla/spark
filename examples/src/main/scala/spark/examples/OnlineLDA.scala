...
object LDAHelpers {
       ...
       def sstats(documents: Array[Document], twd: TopicWordDistribution, expElogbeta: ExpELogBeta,  indices: Array[Int], num_examples: Int, sstats: TopicESStats, alpha: Double, gammas: Array[Array[Double]]) {
...
// Begin eager reduce
class LDAMap(var sstats: TopicESStats, var iter:Int) extends Serializable {
    override def toString: String = {
...
object EagerOnlineLDA {
  type M = LDAMap
  type R = OnlineLDAState
   
  // To the Master 
   class LDAMapMessage (var id:Long, var message: M, @transient theType: R) extends MapMessage[M,R] {
   	 override def toString: String = { return "" } 
   }

    // From the master
    class Diff (var id:Long, @transient message: M, @transient state: OnlineLDAState) extends EagerDiff[M,R] {
	  var update = state.mutable_state
    	  
	  def update(old_var: EagerVar[M,R]) = {
     ...
     object Reducer extends EagerReducer[M,R] {
        ...
        def mapMessage (oldVar: EagerVar[M,R], message: M): MapMessage[M,R] = {
                val dummy:R = null
                return new LDAMapMessage(oldVar.id, message, dummy)
            }
        ...
        def reduceTop (oldVar: EagerVar[M,R], message: M) : EagerDiff[M,R] = {
            ...
            oldVar.value.mutable_state.iter += 1
            var learning_rate = math.pow((oldVar.value.mutable_state.iter + 1024), -.55) 
            var change = LDAHelpers.update_topic_word_dist(oldVar.value.mutable_state.twd, message.sstats, learning_rate, oldVar.value.num_docs, oldVar.value.chunk_size, oldVar.value.eta)

            print("Change: "); println(change)
                    
            if (change < eps && change > 1e-15) {
                oldVar.value.mutable_state.converged = true
            } else {
                oldVar.value.mutable_state.converged = false
            }

            return new Diff(oldVar.id, message, oldVar.value)
        }
     }
}


object EagerOnlineLDAMain {
    ...
    def main(args: Array[String]) {
        ...
        val sc = new SparkContext(args(0), "EagerOnlineLDA")
        val path = args(1)
	val num_words = args(2).toInt
	val num_topics = args(3).toInt
        val slices = if (args.length > 2) args(4).toInt else 1
	...
	var state = sc.eagerReducer(new OnlineLDAState(num_docs, eta, chunk_size, update), EagerOnlineLDA.Reducer)
	...
	for (i <- sc.parallelize(0 until slices, slices)) {
	    ...
	    while (go) {
		  ...
		  //if (max_sstat < 10000) {
		     state.eagerReduce(new LDAMap(sstats.clone, state.value.mutable_state.iter))
		  //} else {
	...
	// Write out the topics to file
	//TicTocLR.appendToFile("EagerOnlineLDA.result", state.value.mutable_state.twd.toString)
        sc.stop()
     }
}
