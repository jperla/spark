...
object MinDoubleEagerVar {
  type D = Double

  class MinDoubleMapMessage (
    var id:Long, var message: D, @transient theType: D) extends MapMessage[D,D]
  {
    override def toString = "id:" + id.toString + ":" + message.toString
  }

  class Diff (
    var id:Long, @transient message: D, @transient theType: D) extends EagerDiff[D,D]
  {
    var myValue = message
    def update(oldVar : EagerVar[D,D]) = {
  ...
  object Reducer extends EagerReducer[D,D] {
    def reduceLocal(oldVar: EagerVar[D,D], message: D) : Boolean = {
    ...
    def reduceTop (oldVar: EagerVar[D,D], message: D) : EagerDiff[D,D] = {
        var doSend = reduceLocal(oldVar, message)
    ...
    def mapMessage (oldVar: EagerVar[D,D], message: D) : MapMessage[D,D] = {
        // todo: why am i sending these things, do i have to? 0.0 transient?!
        return new MinDoubleMapMessage(oldVar.id, message, 0.0)
    }
  }
}
